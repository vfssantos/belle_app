...
//color
#define bottomContainerView_UP_COLOR     [UIColor colorWithRed:21/255.0f green:21/255.0f blue:21/255.0f alpha:0.8f]       //bottomContainerView的上半部分
...
//    [self addPinchGesture];
    ...
    if (![UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        [SVProgressHUD showErrorWithStatus:@"Camera is not supported"];
        
        df_imgView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height)];
        df_imgView.clipsToBounds = YES;
        df_imgView.contentMode = UIViewContentModeScaleAspectFill;
        df_imgView.image = [UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"meizii" ofType:@"jpg"]];
        [self.view addSubview:df_imgView];
    }
#endif
...
- (void)addTopViewWithText:(NSString*)text {
    if (!_topContainerView) {
        CGRect topFrame = CGRectMake(0, 0, SC_APP_SIZE.width, SC_APP_SIZE.height-120-SC_APP_SIZE.width);
        ...
        UILabel *descLbl = [[UILabel alloc] initWithFrame:CGRectMake(0,emptyView.frame.size.height - 56 , SC_APP_SIZE.width, 50)];
        descLbl.text = @"Head above this line";
        descLbl.font = [UIFont fontWithName:@"KohinoorDevanagari-Book" size:15];
        descLbl.numberOfLines = 1;
        descLbl.textAlignment = NSTextAlignmentCenter;
        descLbl.textColor = [UIColor whiteColor];
        [_topContainerView addSubview:descLbl];
        
        UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(10, emptyView.frame.size.height - 1, emptyView.frame.size.width-20, 0.6)];
        lineView.backgroundColor = [UIColor whiteColor];
        [_topContainerView addSubview:lineView];
        
        topFrame.origin.x += 10;
        UIButton * btn = [self buildButton:CGRectMake(10, 0, 44, 44)
                              normalImgStr:@"close_cha.png"
                           highlightImgStr:@"close_cha_h.png"
                            selectedImgStr:nil
                                    action:NSSelectorFromString(@"dismissBtnPressed:")
                                parentView:_topContainerView];
        
        btn.showsTouchWhenHighlighted = YES;
        UIButton * btn1 = [self buildButton:CGRectMake(SC_APP_SIZE.width-50, 0, 44, 44)
...
- (void)addbottomContainerView {
    ...
    CGFloat bottomY = self.view.frame.size.height - 120;
    CGRect bottomFrame = CGRectMake(0, bottomY, SC_APP_SIZE.width, 120);
...
- (void)takePictureBtnPressed:(UIButton*)sender {
    ...
    [_captureManager takePicture:^(UIImage *stillImage) {
        //or your code 2
        NSString *tmpstr = [[NSUserDefaults standardUserDefaults] objectForKey:@"auth_type"];
        if([tmpstr  isEqual: @"1"]){
            PostViewController*con = [[PostViewController alloc] init];
            con.mainImg = stillImage;
            [self.navigationController pushViewController:con animated:YES];

        }
        else{
            PostNoShareViewController*con = [[PostNoShareViewController alloc] init];
            con.mainImg = stillImage;
            [self.navigationController pushViewController:con animated:YES];
        }
    }];
