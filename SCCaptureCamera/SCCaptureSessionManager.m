...
//        [SCCommon saveImageToPhotoAlbum:image];
        ...
        CGFloat headHeight = _previewLayer.bounds.size.height - squareLength-120;//_previewLayer的frame是(0, 44, 320, 320 + 44)
        ...
        CGRect cropFrame = CGRectMake((scaledImage.size.width - size.width) / 2, headHeight*2, size.width, size.height);
...
- (void)switchGrid:(BOOL)toShow {
    ...
    if (!toShow) {
        ...
        for (CALayer *layer in layersArr) {
            if (layer.frame.size.width == 0.5f || layer.frame.size.height == 0.5f) {
    ...
    CGFloat headHeight = _previewLayer.bounds.size.height - SC_APP_SIZE.width-50;
    CGFloat squareLength = SC_APP_SIZE.width;
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (i == 0 || i == 1) {//画横线
            frame = CGRectMake(0, headHeight + (i + 1) * eachAreaLength, squareLength, 0.5f);
        } else {
            frame = CGRectMake((i + 1 - 2) * eachAreaLength, headHeight, 0.5f, squareLength);
        }
